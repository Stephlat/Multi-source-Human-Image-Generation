...
class GAN(object):
    ...
    def __init__(self, generator, discriminator,
                 generator_optimizer=Adam(0.0001, beta_1=.5, beta_2=0.9,clipnorm=1.),
                 discriminator_optimizer=Adam(0.0001, beta_1=.5, beta_2=0.9,clipnorm=1.),
    ...
    def _set_trainable(self, net, trainable,only_dec=False):

        for layer in net.layers:

 
            if only_dec and ("Encoder" in layer.name or "encoder" in layer.name):
            ...
            if hasattr(layer, 'layers'):
                for layer2 in layer.layers:
                    if only_dec and ("Encoder" in layer2.name or "encoder" in layer2.name):
    ...
    def compile_models(self,dec_only=False):
        ...
        return self._compile_generator(dec_only), self._compile_discriminator()
