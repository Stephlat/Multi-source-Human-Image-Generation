...
class Trainer(object):
    def __init__(self, dataset, gan, output_dir = 'output/generated_samples',
                 ...
                 number_of_epochs=100, batch_size=64,nb_rec=0, **kwargs):
        ...
        generator_model, discriminator_model = gan.compile_models(kwargs['train_dec_only'])
    ...
    def save_generated_images(self):
        ...
        image = self.dataset.display(self.generator.predict_on_batch(batch), batch,self.nb_inputs)
    ...
    def train_one_step(self, discriminator_loss_list, generator_loss_list):
        for j in range(self.training_ratio):
            ...
            #                 discrimiantor_batch + generator_batch, [np.zeros([self.batch_size])]*(self.nb_inputs+self.nb_rec)*self.nb_inputs) # numer of generated images X number of conditioning image
            loss = self.discriminator_model.train_on_batch(
                            discrimiantor_batch + generator_batch, np.zeros([self.batch_size])) # numer of generated images X number of conditioning image
    ...
    def train_one_epoch(self, validation_epoch=False):
        ...
        #             # loss = self.generator_model.test_on_batch(generator_batch, [np.zeros([self.batch_size])]*(self.nb_inputs+self.nb_rec)*self.nb_inputs)
    ...
    def train(self):
        while (self.current_epoch < self.last_epoch):            
            ...
            self.train_one_epoch((((self.current_epoch + 1) % self.checkpoint_ratio == 0)))
